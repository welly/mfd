...
final class ToolkitStep implements Step
{
     ...
     * of stderr. Keep the last 1-3 non-empty stderr lines, the actual reason, instead of just the
     * "exited N" header, which carries no information about what went wrong.
     */
    private static function reason(CommandFailed $e): string
    {
        $lines = explode("\n", $e->getMessage());
        $header = array_shift($lines);
        $stderr = array_values(array_filter($lines, static fn (string $line): bool => trim($line) !== ''));

        return $stderr === [] ? rtrim($header, ':') : implode('; ', array_slice($stderr, -3));
    }
